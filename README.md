...
- **本地化适配优化**：修复时区、字符兼容、页面渲染问题，适配 DSH 侧边栏工作台

## 三、核心功能

- **对话一键成图**：任意对话点击标题旁图谱按钮，自动生成包含节点、实体、关系、工具调用的完整知识图谱，支持刷新、独立窗口放大、浏览器原生打开
...
- **自动铺开宽度**：打开图谱面板时，侧边栏若窄于 640px 会自动加宽（只加宽不回缩，你自己拖过的宽度会被尊重）
...
- 必备插件：**dsh\-better\-sidebar**（提供侧边栏展示容器）
...
打开任意 AI 对话 → 点击会话标题右侧**图谱按钮** → 右侧自动展开图谱工作台，等待几秒即可生成完整对话知识图谱。

支持重新抽取、刷新图谱、独立窗口放大、AI 一键分析等所有功能。
