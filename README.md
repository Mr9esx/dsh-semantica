...
**「新对话默认」出厂就是开的**（面板工具栏里那个 `新对话默认：开/关`，紧挨着
`每轮提取` —— 前者管以后新开的对话，后者管当前这个）。想让它别自动写，点一下关掉即可。

为什么默认开，而不是像早期那样默认关：用户报过「打开知识图谱，里面跟我这个对话完全
没关系」—— 查下来那个对话**一个字都没写进去**（一轮纯知识问答，模型按「值得记才记」
的规则没写），而默认关意味着「打开面板什么都看不到」。默认关省 token，默认开省心；
用户选了后者。老配置文件会自动升级（文件里没有 `v` 字段 = 旧版本写的，那个 `default:
false` 是旧代码的默认值、不是用户的选择），升级后想改回来点一下就行。
...
| `新对话默认：开/关` | 工具栏里的小开关，挨着 `每轮提取`：让**以后每条新对话一开始就**自动提取（出厂就是开） |
| 图文件路径（等宽字样，如 `…/dsh-semantica-graph/kg.json`） | **点一下复制完整路径**（鼠标是 copy 光标；tooltip 里有完整路径，以及这个路径是哪来的 —— 见下面第 8 条） |
...
**3.5 面板里显示「本对话在图里还没有节点」怎么办。** 这**不是**图坏了，是这个对话还没
写过东西（图里那些是别的会话写进同一个图库的，切「全部」才看得到）。面板空态里直接给了
两步：① 点「复制提取指令」；② 粘到输入框发出去 —— 模型当场就把这次对话抽进图。想以后
每轮自动写，就开「每轮提取」（新对话默认已经是开的）。
...
**8. 图文件路径是谁定的。** 决定 MCP 子进程读写哪个文件的是 profile 里写死的
`SEMANTICA_KG_PATH`（安装脚本写进去的），所以插件**也以它为准**：面板上显示并让你复制
的那个路径是 host 从 `<harness>/profiles/*/cordis.patch.yml` 里读出来的真实值，不是插件
自己按约定拼的。取值顺序是「profile 里写的 → 插件进程自己的同名环境变量 → 安装脚本的
默认约定」；悬停路径按钮的 tooltip 会说明它来自哪里（`profile「web」` / 环境变量 / 默认）。
你手改过路径的话，面板显示的会跟着变，插件读的也是那一份。

**9. 插件不碰会话标题那一排。** 标题旁边没有插件的按钮（曾经放过「打开知识图谱」和
同名开关，已经按用户要求去掉）—— 打开面板就点标签本身。

**10. 只想让某几个会话开着**：开关是**按会话**存的，新会话跟着「新对话默认」（默认开）。
