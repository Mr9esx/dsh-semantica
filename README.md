...
**2. 想让模型每轮都写，就把「每轮提取」开关打开。** 会话标题右侧有个
`每轮提取：关 / 开` 的按钮（知识图谱面板的工具栏里也有一个，两处是同一个开关）：

- **关着**（默认）：模型只在它自己判断值得记的时候写（你确认过的选择、定下的方案、
  以后还要复用的结论）。想让它记，直接说「把刚才这个决定记进知识图谱」。
- **开着**：每一轮回复结束前，模型都必须把这一轮的新知识写进图谱。**代价是每轮都会
  多几次 MCP 工具调用（多花 token）**，所以默认不开；查资料、闲聊这种不产生知识的
  轮次它会跳过，插件也不会替它判断。

开关的作用机制：它改的是**下一轮**的系统提示词（`semantica_directive` 这个变量每次
组装提示词时重新求值），所以点完立刻生效，不用重启、也不用再往对话里粘指令。
**它替代的就是以前那个「复制提取指令 → 粘进对话」的动作** —— 那个按钮还在（补写历史
对话时还用得上），但日常不需要了。

**图不是聊天记录的转录**，只是「值得以后复用的知识」。没说过、模型也没判断值得记的
对话，图里就是没有 —— 这是设计，不是故障。

**3. 看。** 对话顶部标签里的「**知识图谱**」（会话标题右侧也有个同名入口按钮）。
进去以后：
...
| `每轮提取：关/开` | 会话标题右侧和面板工具栏各一个，同一个开关：开了之后每轮都写图（见上面第 2 条） |
...
**8. 只想让某几个会话开着**：开关是**按会话**存的（`<harness>/dsh-semantica-graph/auto-extract.json`），
新会话默认关，不会因为上个会话开着就跟着开。
