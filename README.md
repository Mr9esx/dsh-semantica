...
**2. 想让模型每轮都写，就把「每轮提取」开关打开。** 开关有**三处入口**，都是同一个：

| 位置 | 什么时候看得见 |
| --- | --- |
| **输入框那一排右侧**（`图谱提取 关/开`，紧凑样式） | 聊天时——就在你打字的地方 |
| 会话标题右侧（`每轮提取：关/开`） | 一直看得见（切到「知识图谱」标签时输入框会被藏起来，这处还在） |
| 知识图谱面板的工具栏 | 面板里 |

**想让「每条新对话」一开始就自动提取**：面板里点一下 `新会话默认：关 → 开`。
...
| `每轮提取：关/开` | 同一开关，另外两处入口：会话标题右侧、面板工具栏 |
| `新会话默认：关/开` | 面板下方那行里的小开关：让**以后每条新对话一开始就**自动提取 |
...
**8. 只想让某几个会话开着**：开关是**按会话**存的，新会话跟着「新会话默认」。
