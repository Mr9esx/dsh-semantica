...
**2. 图里的内容由「说出来」和「模型自己判断」两种方式产生。** 记住这条，否则会觉得
「我聊了半天怎么是空的」：

- 你明确说：**「把刚才这个决定记进知识图谱」** —— 最可靠，想记就直说；
- 模型自己判断（提示词里写了这些触发条件）：你确认过的选择、定下方案/约定、
  得出以后还要复用的结论时，顺手记一条。
...
| 数字下面那行 | 归属说明：哪些决策是按实体认领的、哪些是按时间认领的、图里还有多少节点没打标 |
...
| 面板上看到 | 意思 / 怎么办 |
| --- | --- |
| 插件的 host 半侧还没加载 | 改过 host 代码（`src/index.js` 等）→ 重启 DSH Desktop |
| 这台机器上还没有写过任何知识图谱 | 图还没建过 → 点「复制提取指令」粘给模型 |
| 本对话在图里还没有节点 | 模型没给这次对话写，或没打标 → 点「复制提取指令」 |
